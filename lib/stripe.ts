...
/**
 * Creates a real Stripe Checkout Session for one course purchase. RWF is
 * a real zero-decimal currency Stripe supports natively (no cents
 * multiplication needed, unlike USD), so the course's own amountRwf is
 * used directly as unit_amount — keeps course pricing in the same unit
 * PayPack already uses instead of introducing a second currency
 * convention alongside it.
 */
export async function createCheckoutSession({ courseOrderId, courseTitle, amountRwf, successUrl, cancelUrl }: CreateCheckoutSessionInput): Promise<{ id: string; url: string }> {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    metadata: { courseOrderId },
    payment_intent_data: { metadata: { courseOrderId } },
  })
