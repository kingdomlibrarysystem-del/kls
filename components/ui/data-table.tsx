...
export interface DataTableProps<T> {
  ...
  filters?: React.ReactNode          // extra filter controls rendered in the toolbar
  pageSizeOptions?: number[]
  defaultPageSize?: number
  emptyMessage?: string
  onExport?: () => void
  caption?: string                   // shown above table as result count label
}
...
function SortIndicator({ active, dir }: { active: boolean; dir: SortDir }) {
  if (!active) return <ChevronUp size={12} className="text-w-400" />
...
function PaginationBar({
  ...
  const btnBase = 'border border-w-300 rounded font-lato text-xs text-w-700 hover:bg-w-100 disabled:opacity-40 disabled:cursor-not-allowed transition-colors'
  ...
  return (
    <div className="flex items-center justify-between mt-4">
      <p className="font-lato text-xs text-w-700">Page {page} of {totalPages}</p>
      <div className="flex items-center gap-1">
        ...
        {pills.map((n, i) =>
          n === '…' ? (
            <span key={`e${i}`} className="px-1.5 font-lato text-xs text-w-600">…</span>
          ) : (
            <button
              key={n}
              onClick={() => onPage(n as number)}
              className={`px-2.5 py-1 rounded text-xs font-lato border transition-colors ${
                page === n ? 'bg-w-600 text-white border-w-600' : 'border-w-300 text-w-700 hover:bg-w-100'
              }`}
            >
              {n}
            </button>
...
export function DataTable<T>({
  ...
  return (
    <div>
      {/* Toolbar */}
      <div className="bg-form-highlight border border-w-300 rounded-lg p-3 mb-3 flex flex-wrap gap-3 items-center justify-between">
        ...
        {onExport && (
          <button
            onClick={onExport}
            className="flex items-center gap-1.5 px-3 py-2 border border-w-400 bg-white rounded font-lato text-sm text-w-700 hover:bg-w-100 transition-colors"
          >
      ...
      {/* Result count */}
      <p className="font-lato text-xs text-w-700 mb-2">
      ...
      <div className="bg-white border border-w-300 rounded-lg overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-form-section border-b border-w-300">
              <tr>
                {columns.map((col) => (
                  <th
                    key={col.key}
                    onClick={col.sortable ? () => handleSort(col.key) : undefined}
                    className={`px-4 py-3 text-left font-lato text-xs font-semibold text-w-700 uppercase tracking-wider ${col.sortable ? 'cursor-pointer select-none hover:text-w-950 transition-colors' : ''} ${col.className ?? ''}`}
                  >
            ...
            <tbody className="divide-y divide-w-200">
              {paged.length === 0 ? (
                <tr>
                  <td colSpan={columns.length} className="px-4 py-12 text-center font-lato text-sm text-w-700">
                    {emptyMessage}
                  </td>
                </tr>
              ) : (
                paged.map((row) => (
                  <tr key={rowKey(row)} className="hover:bg-form-highlight transition-colors">
                    {columns.map((col) => (
                      <td key={col.key} className={`px-4 py-3 font-lato text-sm text-w-950 ${col.className ?? ''}`}>
                        {col.render(row)}
                      </td>
                    ))}
                  </tr>
