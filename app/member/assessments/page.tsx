  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5" style={{ display: "grid", gap: 10 }}>
        {[
          { icon: <ClipboardList size={18} />, label: "Total Taken", value: attempts.length.toString(), color: "var(--text-primary)" },
          { icon: <CheckCircle2 size={18} />, label: "Passed", value: passed.length.toString(), color: "var(--green-light)" },
          { icon: <XCircle size={18} />, label: "Failed", value: failed.length.toString(), color: "var(--red-light)" },
          { icon: <Clock size={18} />, label: "Under Review", value: underReview.length.toString(), color: "var(--gold)" },
          { icon: <FileText size={18} />, label: "Avg Score", value: `${avgScore}%`, color: "var(--gold)" },
        ].map((s) => (
      ...
      {pending.length > 0 && (
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
          <div style={{ padding: "12px 14px", borderBottom: "1px solid var(--border)", fontSize: 14, fontWeight: 700, color: "var(--text-primary)", display: "flex", alignItems: "center", gap: 6 }}>
            <Clock size={16} color="var(--gold)" /> Upcoming Assessments
          </div>
          {pending.map((a) => (
            <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 14px", borderBottom: "1px solid var(--border-light)", flexWrap: "wrap" }}>
              <div style={{ width: 40, height: 40, borderRadius: 8, background: "rgba(212,168,67,0.1)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <FileText size={18} color="var(--gold)" />
              </div>
              <div style={{ flex: 1, minWidth: 160 }}>
                <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{a.title}</div>
                <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{courseTitleFor(a.id)}</div>
              </div>
              <Link
      ...
      {attempts.length > 0 ? (
        <div style={{ background: "var(--bg-card)", border: "1px solid var(--border)", borderRadius: 8, overflow: "hidden" }}>
          <div style={{ padding: "12px 14px", borderBottom: "1px solid var(--border)", fontSize: 14, fontWeight: 700, color: "var(--text-primary)" }}>
            Assessment History
          </div>
          {attempts.map((a) => {
            ...
            return (
              <div key={a.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "8px 14px", borderBottom: "1px solid var(--border-light)" }}>
                <div style={{ width: 40, height: 40, borderRadius: 8, background: "var(--bg-section)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                  {isPending ? <Clock size={18} color={statusColor} /> : a.status === "PASSED" ? <CheckCircle2 size={18} color={statusColor} /> : <XCircle size={18} color={statusColor} />}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: "var(--text-primary)" }}>{takeableAssessments[a.assessmentId]?.title ?? a.assessmentId}</div>
                  <div style={{ fontSize: 11, color: "var(--text-muted)" }}>{courseTitleFor(a.assessmentId)} • {a.takenAt}</div>
                </div>
