...
export function EditLessonModal({ lesson, onClose }: EditLessonModalProps) {
  ...
  useEffect(() => {
    if (lesson) {
      reset({ title: lesson.title, contentType: lesson.contentType, durationMinutes: lesson.durationMinutes, content: lesson.content })
    }
  }, [lesson, reset])
  ...
  return (
    <Modal open={!!lesson} onClose={onClose} title="Edit Lesson" size="md">
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div>
          <FieldLabel htmlFor="edit-lesson-content" required>Content</FieldLabel>
          <textarea
