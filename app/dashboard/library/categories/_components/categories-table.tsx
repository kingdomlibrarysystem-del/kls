...
/** DataTable of categories with View/Edit/Delete row actions, extracted from the original page.tsx (View added). */
export function CategoriesTable({ categories, onView, onEdit, onDelete }: CategoriesTableProps) {
  const columns: Column<Category>[] = [
    ...
    },
    {
      ...
      render: (c) => <span className={`font-cinzel font-bold text-sm ${c.resourceCount > 0 ? 'text-w-600' : 'text-w-400 dark:text-white/30'}`}>{c.resourceCount}</span>,
    },
    {
