...
interface DeleteCategoryModalProps {
  ...
  /** Live-computed resource count for this category (recursive for roots) — see `resourceCountFor` in lib/kcs-taxonomy. */
  resourceCount: number
  onClose: () => void
  onConfirm: () => void
}

/** Delete confirmation modal — blocks deletion while `resourceCount` (now computed live, not a stored field) is above 0. */
export function DeleteCategoryModal({ category, resourceCount, onClose, onConfirm }: DeleteCategoryModalProps) {
  return (
      ...
      {category && (
        <div>
          ...
          <div className="flex gap-2">
            <ElegantButton variant="primary" onClick={onConfirm} disabled={resourceCount > 0} className="flex-1 text-sm py-2 bg-red-600 border-red-700 hover:bg-red-700">
              Delete
            </ElegantButton>
