  ...
  const columns: Column<Category>[] = [
      ...
      render: (c) => {
        ...
        return (
          <div className="flex items-center gap-2">
            <div className="w-7 h-7 rounded bg-w-100 dark:bg-white/10 flex items-center justify-center shrink-0 overflow-hidden relative">
              <RemoteImage
                src={c.heroImage}
                alt=""
                fill
                ...
                fallback={<FolderOpen size={13} className="text-w-600" />}
              />
            </div>
            <div>
              <p className="font-semibold text-w-950 dark:text-white text-sm">
                {c.name.en}
              </p>
              {parentName && (
    ...
    },
    {
      ...
      render: (c) => {
        ...
        return (
          <span
            className={`font-cinzel font-bold text-sm ${count > 0 ? "text-w-600" : "text-w-400 dark:text-white/30"}`}
          >
            {count}
          </span>
        );
    ...
    },
    {
      ...
      sortable: true,
      render: (c) => (
        <span className="font-lato text-xs text-w-600 dark:text-white/40">
          {c.createdAt}
        </span>
      ),
    },
