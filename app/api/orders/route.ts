 ...
 * Transaction[]. Creating an order immediately requests a real PayPack
 * mobile-money cashin (moves real RWF the instant this succeeds — no
 * sandbox exists, see lib/paypack.ts). The order starts PENDING; only
 * the webhook at /api/webhooks/paypack (or a manual status refresh)
 * flips it to PAID once PayPack actually confirms the charge.
 */
